...
public:
  static const uint32_t MAX_AIRPORT_COUNT 
	  = static_cast<uint32_t>(TAirport::Last_); 
  using Item = pair<TAirport, size_t>;
  using Items = const array<Item, MAX_AIRPORT_COUNT>&;

  
private:
  array<Item, MAX_AIRPORT_COUNT> statistics; /// may use array<size_t, ...>

public:
  // конструктор по умолчанию: список элементов пока пуст
  AirportCounter() {
    for (size_t i = 0; i < MAX_AIRPORT_COUNT; ++i) {
      statistics[i].first = static_cast<TAirport>(i);
      //statistics[i].second = 0;   /// wastes time , done defaultly
      // but we can use method ... statistics.fill(0)  (if we have array<size_t>)
    }
  }
  ...
  AirportCounter(TIterator begin, TIterator end) {
    ...
    for (auto it = begin; it != end; ++it) {
      statistics[static_cast<uint32_t>(*it)].second ++;
    }
  }

  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const {
    uint32_t port = static_cast<uint32_t>(airport);
    return statistics[port].second;
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
    uint32_t port = static_cast<uint32_t>(airport);
    statistics[port].second ++;
  }

  // удалить одно вхождение данного элемента
  void EraseOne(TAirport airport) {
    uint32_t port = static_cast<uint32_t>(airport);
    if (statistics[port].second > 0) {
      statistics[port].second --;
    }
  }
