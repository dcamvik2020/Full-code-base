...
public:
    ...
    bool operator == (const Matrix<T>& x) const;
    void MakeAccuracy(); /// rounds all x : |x| < 1e-10 in Matrix<T> to 0
    ...
    void add_column(const Vector<T>& v);
    void add_row(const Vector<T>& v);
...
template <class T>
void Matrix<T>::add_column(const Vector<T>& v) {
    if (rows != v.size()) {
        throw std::runtime_error("Matrix-Vector sizes don't match");
    }
    ++cols;
    //a.resize(rows);
    for (unsigned i = 0; i < rows; ++i) {
...
template <class T>
void Matrix<T>::add_row(const Vector<T>& v) {
    if (cols != v.size()) {
        throw std::runtime_error("Matrix-Vector sizes don't match");
    }
    //a.resize(rows);
...
template <class T>
int Matrix<T>::rg() const {
    Matrix<T> mtr(*this);

    unsigned i, j, k, column;
    T x;
    Vector<T> row;
    ///mtr -> step type of matrix
    for (i = 0, column = 0; i < rows && column < cols; ++column) {
        if (!cor0(abs(mtr[i][column]))) {
            for (j = i + 1; j < rows && (!cor0(abs(mtr[j][column]))); ++j);
            if (j == rows) {
                continue;
            }
            row = mtr[j];
