...
/// In programmer's responsibility is to remember about :
/// - Vector sizes (he can change it as he wants at any time)
/// - Vector's aim (what variable responses)
...
private:
    ...
    unsigned real_size;  /// used memory
    T *a;
...
public:
  ...
  void push(const T& x);
  T pop(); 
  unsigned insert(const T& x); /// in sorted order
  ...
  friend Vector<T> operator+ (Vector<T> x, const Vector<T> & y) {return x += y;}
  friend Vector<T> operator- (Vector<T> x, const Vector<T> & y) {return x -= y;}
  friend Vector<T> operator* (Vector<T> x, double p)            {return x *= p;}
  friend Vector<T> operator/ (Vector<T> x, double p)            {return x /= p;}
  ...
  friend std::ostream& operator << (std::ostream& out, const Vector<T> & x) {
    out << "size of vector : " << x.N << std::endl;
    out << "vector : ";
    for (unsigned i = 0; i < x.N; ++i) {
      out << x[i] << " ";
    }
...
template <class T>
unsigned Vector<T>::insert(const T& x) {
  unsigned i, index;
  if (N == real_size) {
    ++real_size;
    T *b = new T[N+1];
...
template <class T>
void Vector<T>::insert(const T& x, const unsigned index) {
  if (index > N) {
...
template <class T>
unsigned Vector<T>::resize(unsigned new_size) {
...
template <class T>
unsigned Vector<T>::find(const T& x) const {
...
template <class T>
Vector<T>& Vector<T>::operator *= (double p) {
  for (unsigned i = 0; i < N; ++i) {
    a[i] *= p;
  }
  return *this;
...
template <class T>
T Vector<T>::erase(const unsigned& index) {
  if (N == 0 || index >= N) {
...
std::ostream& operator << (std::ostream& out, const std::vector < T > & x) {
    out << "size of vector : " << x.size() << std::endl;
    out << "vector : ";
