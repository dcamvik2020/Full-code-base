...
ForwardIterator max_element_if(ForwardIterator first, ForwardIterator last, UnaryPredicate pred) {
  ...
  ForwardIterator needed_max_it = last;
  ForwardIterator it;                       // = find_if(first, last, [pred](const typename ForwardIterator::value_type & x){return pred(x);});
  for (it = first ; it != last; it++) {
    if (pred(*it) == true) {
      if (first_needed == true) {
        needed_max_it = it;
	first_needed = false;
      } else if (*needed_max_it < *it) {
        needed_max_it = it;
      }
    }
  }
  return needed_max_it; 
}
