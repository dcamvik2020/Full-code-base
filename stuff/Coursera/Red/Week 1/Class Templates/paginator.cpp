...
public:
  Paginator(Iterator begin, Iterator end, size_t page_size) {
    Iterator first, last; /// for any page
    for (Iterator it = begin; it != end; it = last) {
      first = it;
      if (size_t(end - it) < page_size) {
	last = end;
      } else {
        last = first;
        advance(last, page_size);
      }
      if (first != last) {
        Pages.push_back(IteratorRange<Iterator>{first, last});
      }
    }
  }
...
template <typename C>
auto/*Paginator<typename C::iterator>*/
